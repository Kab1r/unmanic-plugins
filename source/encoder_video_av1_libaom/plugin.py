...
from encoder_video_av1_libaom.lib.ffmpeg import StreamMapper, Probe, Parser

# Configure plugin logger
logger = logging.getLogger("Unmanic.Plugin.encoder_video_av1_libaom")
...
class PluginStreamMapper(StreamMapper):
    ...
    def custom_stream_mapping(self, stream_info: dict, stream_id: int):
        if self.settings.get_setting('advanced'):
            stream_encoding = ['-c:v:{}'.format(stream_id), 'libaom-av1']
            stream_encoding += self.settings.get_setting('custom_options').split()
        else:
            stream_encoding = [
                '-c:v:{}'.format(stream_id), 'libaom-av1',
...
def on_worker_process(data):
    ...
    if mapper.streams_need_processing():
        ...
        if settings.get_setting('keep_container'):
            ...
            aomainer_extension = settings.get_setting('dest_container')
